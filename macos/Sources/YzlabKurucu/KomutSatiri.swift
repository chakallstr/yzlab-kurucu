...
/// Bassiz (headless) kurulum: `YzlabKurucu --kur --anahtar yzk_live_… [--model id] [--kisayol 0|1]`
...
enum KomutSatiri {
    ...
    static func calistir(_ argv: [String]) -> Never {
        Task { @MainActor in
            ...
            print("manifest: \(canli ? "canli" : "gomulu")")
            ...
            guard let anahtar = deger("--anahtar", argv) ?? env["YZLAB_ANAHTAR"],
                ...
                print("kullanim: --kur --anahtar <yzk_live_…> [--model <id>] [--kisayol 0|1]")
            ...
            do {
                try await k.kur(anahtar: anahtar, model: model, kisayol: kisayol)
